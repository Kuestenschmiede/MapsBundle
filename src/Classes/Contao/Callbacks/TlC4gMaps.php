...
//        $image = $this->{$callback[0]}->{$callback[1]}($row,'');
    ...
    {
        return ' <a href="'.System::getContainer()->get('router')->generate('contao_backend') .'/picker?context=page&value='.str_replace(array('{{link_url::', '}}'), '', $dc->value).'" title="'.StringUtil::specialchars($GLOBALS['TL_LANG']['MSC']['pagepicker']).'" onclick="Backend.getScrollOffset();Backend.openModalSelector({\'width\':765,\'title\':\''.StringUtil::specialchars(str_replace("'", "\\'", $GLOBALS['TL_LANG']['MOD']['page'][0])).'\',\'url\':this.href,\'id\':\'tl_listing\',\'tag\':\'ctrl_'.$dc->field . ((Input::get('act') == 'editAll') ? '_' . $dc->id : '').'\',\'self\':this,\'callback\': (param1, value) => { document.querySelector(\'#ctrl_'.$dc->field.'\').value = \'{{link_url::\'+value[0]+\'}}\'; }});return false">' . Image::getHtml('pickpage.svg', $GLOBALS['TL_LANG']['MSC']['pagepicker'], 'style="vertical-align:top;cursor:pointer"') . '</a>';
    }
