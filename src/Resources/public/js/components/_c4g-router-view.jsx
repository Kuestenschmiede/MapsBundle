...
export class RouterView extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      detourRoute: (props.detourRoute && props.detourRoute.initial) ? props.detourRoute.initial : false,
      detourArea: (props.detourArea && props.detourArea.initial) ? props.detourArea.initial : false,
      featureList: {
        features: [],
        type: ""
      },
  ...
  render() {
    ...
    if (this.state.mode === "route") {
      sliderOptions = {
        min: (mapData.detourRoute && mapData.detourRoute.min) ? mapData.detourRoute.min : 0,
        max: (mapData.detourRoute && mapData.detourRoute.max) ? mapData.detourRoute.max : 100,
        value: this.state.detourRoute,
        router: this
      };
    } else if (this.state.mode === "area") {
      sliderOptions = {
        min: (mapData.detourArea && mapData.detourArea.min) ? mapData.detourArea.min : 0,
        max: (mapData.detourArea && mapData.detourArea.max) ? mapData.detourArea.max : 100,
