      ...
      switch (baseLayerConfig.provider) {
        ...
        case 'osm':
          if (sourceConfigs.osm[baseLayerConfig.style]) {
            newBaselayer = new ol.layer.Tile({
              source: new ol.source.OSM(jQuery.extend(sourceConfigs.osm[baseLayerConfig.style], layerOptions))
            });
        ...
        case 'stamen':
          if (sourceConfigs.stamen[baseLayerConfig.style]) {
            // Stamen
            newBaselayer = new ol.layer.Tile({
              source: new ol.source.Stamen(jQuery.extend(sourceConfigs.stamen[baseLayerConfig.style], layerOptions))
            });
        ...
        case 'here':
          if (baseLayerConfig.api_key && baseLayerConfig.app_id && baseLayerConfig.here_type) {
            ...
            newBaselayer = new ol.layer.Tile({
              source: new ol.source.XYZ(jQuery.extend(sourceConfigs.here[baseLayerConfig.here_type], layerOptions))
            });
          } else if (baseLayerConfig.hide_in_be) {
            layerOptions.url = layerOptions.url = "con4gis/baseLayerTileService/" + baseLayerConfig.id + "/{z}/{x}/{y}";
            newBaselayer = new ol.layer.Tile({
              source: new ol.source.XYZ(jQuery.extend(sourceConfigs.mapbox[baseLayerConfig.here_type], layerOptions))
            });
          } else {
            ...
            if (fFeatures[0].get('cluster_popup') == 1) {
              ...
              if (currentZoom >= minZoom) {
                ...
                for (var i = 0; i < max; i++) {
                  ...
                  var f = [];
                  var featureLinestring = new ol.Feature(new ol.geom.LineString([newCenter, p]));
                ...
                var stringSource = new ol.source.Vector({ features: arrLinestring });
                ...
                var stringLayer = new ol.layer.Vector({
                  source: stringSource,
                  style: stringStyle
                });
